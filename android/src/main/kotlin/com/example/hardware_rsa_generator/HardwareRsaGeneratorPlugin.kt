...
class HardwareRsaGeneratorPlugin: FlutterPlugin, MethodCallHandler {
  ...
  private fun generateKeyPair() {
    try {
      ...
      val baseSpecBuilder = KeyGenParameterSpec.Builder(
        keyAlias, // Alias to identify the key
        KeyProperties.PURPOSE_SIGN or KeyProperties.PURPOSE_VERIFY // Allow signing and verifying
      )
        .setDigests(KeyProperties.DIGEST_SHA256, KeyProperties.DIGEST_SHA512) // Supported hash algorithms
        .setSignaturePaddings(KeyProperties.SIGNATURE_PADDING_RSA_PKCS1) // Use PKCS#1 padding
        .setKeySize(3072) // Key size in bits
      ...
      val strongBoxSpec = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        baseSpecBuilder.setIsStrongBoxBacked(true).build()
      } else {
        throw UnsupportedOperationException("VERSION.SDK_INT < P") // StrongBox not available on API < 28
      }
  ...
  private fun getPublicKey(): String {
    ...
    privateKeyEntry?.let {
      ...
      return Base64.encodeToString(publicKey.encoded, Base64.DEFAULT) // Encode public key as Base64 string
    } ?: run {
  ...
  private fun signData(data: ByteArray): String {
    try {
      ...
      val signature = Signature.getInstance("SHA256withRSA") // You can change this if you prefer another algorithm like "SHA512withRSA"
